...
import { default as rproxies } from "./http_proxies.json" with { type: "json" };
...
async function pFetch(url, fetchParams, pi = 0) {
	...
	if (proxies[pi].ssl!==true) return pFetch(url, fetchParams, pi+1);
	try {
		const agent = new SocksProxyAgent(proxies[pi].proxy);
